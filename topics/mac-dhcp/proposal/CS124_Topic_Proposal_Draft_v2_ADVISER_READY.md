...
> **Although prior work has established that Randomized and Changing MAC (RCM) behavior can affect DHCP operation and has separately investigated DHCP lease-time and address-pool efficiency, the literature reviewed thus far does not provide a controlled quantitative characterization of how MAC identity turnover interacts with DHCP Client-Identifier continuity, lease duration, and constrained address-pool pressure. Consequently, the operating conditions under which DHCP-visible identity turnover produces negligible, moderate, or service-impacting lease amplification remain insufficiently characterized.**
...
3. **To design and develop** a reproducible DHCPv4 simulation-and-validation prototype that integrates the discrete-event model with an isolated Kea DHCPv4 environment and programmable client identity generation for verifying selected identity, lease, renewal, release, saturation, and allocation-failure scenarios.
...
1. How do standards-informed MAC identity profiles affect DHCPv4 lease utilization under varying lease durations and address-pool pressure?
...
# Evidence Supporting Proposal Feasibility

The proposal has passed a controlled implementation-level mechanism gate using Kea DHCPv4 3.2.0 and Scapy 2.7.0 in WSL2. The purpose of this pilot was not to produce the main research findings, but to test whether the simulator's proposed DHCP identity semantics are implementable and observable.

| Validation | Controlled condition | Observed server outcome | Model implication |
|---|---|---|---|
| V0 | same MAC + same Client-ID | same address reused; 1 assigned lease | baseline lease continuity |
| V1 | changing MAC + persistent Client-ID | same address reused; 1 assigned lease | MAC turnover alone need not amplify leases |
| V2 | changing MAC + changing Client-ID | 3 identities produced 3 simultaneous leases | DHCP-visible identity turnover can amplify lease state |
| V3 | 4 identities against a 3-address pool | saturation at 3/3; fourth request failed | saturation and allocation failure must be separate metrics |
| V4 | changing identity + valid DHCPRELEASE | final assigned leases returned to 0 | explicit release reduces retained lease pressure |
| V5 | changing MAC + same Client-ID with Client-ID matching disabled | 3 MACs produced 3 leases | server identity-matching policy changes the effect of MAC turnover |

**Proposal use:** Present these only as pilot/mechanism validation and feasibility evidence. Do not present them as the final quantitative answer to the research questions.

---

# Literature Positioning Matrix

| Literature stream | What it already establishes | What this thesis must NOT claim | Role in this thesis |
|---|---|---|---|
| DHCP standards (RFC 2131, RFC 4361) | DHCPv4 has client identity semantics beyond MAC alone; Client Identifier can be independent of hardware address | that DHCP always identifies clients solely by MAC | defines model identity rules |
| DHCP privacy profile (RFC 7844) | MAC, IP, and DHCP identifiers interact with privacy and linkability | that Client-ID/MAC interaction is newly discovered | motivates identity-continuity factor |
| Modern RCM standards (RFC 9724, RFC 9797) | RCM has multiple behavioral forms and can disrupt network state/services | that modern devices rotate MAC on every reconnect | motivates standards-informed identity profiles |
| Ficara et al. survey | DHCP lease management, pool exhaustion, and identifier/privacy tension are recognized RCM issues | that RCM affecting DHCP pools is a new observation | strongest contemporary problem/gap anchor |
| DHCP lease optimization literature | lease duration affects address utilization and protocol overhead | that lease-time optimization is novel | establishes lease-duration trade-off |
| Large-scale DHCP measurement/modeling | real WLAN DHCP behavior has been empirically measured and modeled | that DHCP simulation/modeling itself is novel | methodology and workload context |
| Kea mechanism validation | selected identity/lease behaviors can be reproduced in a real DHCP implementation | that one Kea lab proves prevalence in production networks | implementation-validity layer |

---

# Adviser / Panel Defense Positioning

**If asked, “Isn’t it obvious that more identities mean more leases?”**

The study does not claim the existence of that mechanism as the research contribution. It quantifies the conditions under which physical-client identity turnover becomes DHCP-visible lease amplification, and estimates interaction and threshold effects across Client-Identifier continuity, lease duration, and pool pressure.

**If asked, “Has DHCP lease optimization already been studied?”**

Yes. Lease-time optimization is mature prior work. This thesis is not proposing a new adaptive lease optimizer. Lease duration is an experimental factor used to characterize the interaction between identity turnover and constrained pool state.

**If asked, “MAC randomization affecting DHCP is already known, so what is new?”**

Correct. The proposed contribution is narrower: a reproducible, implementation-validated quantitative characterization of the joint interaction among MAC identity turnover, Client-Identifier continuity, lease duration, and pool pressure.

**If asked, “Why is this Computer Science rather than just networking configuration?”**

The primary output is a standards-informed discrete-event computational model, a reproducible simulation-and-validation framework, and an interaction/threshold analysis. The software is the experimental vehicle rather than the contribution itself.

**If asked, “Why no machine learning?”**

The research question concerns controlled state transitions, resource occupancy, and factor interactions. A discrete-event model represents those mechanisms directly; introducing machine learning would not inherently improve validity or answer the causal/interaction question.

**If asked, “What if no allocation failures occur in many conditions?”**

That is a valid outcome. Safe operating regions are part of the expected contribution. The study separately measures lease amplification, saturation, and actual allocation failure, so it does not require every condition to fail.

---

# Proposal-Safe Terminology

Prefer:
- **DHCP-visible identity turnover**
- **Client-Identifier continuity**
- **valid lease occupancy**
- **pool saturation**
- **allocation failure**
- **valid-lease-to-active-client ratio**
- **standards-informed identity profile**
- **implementation-level mechanism validation**

Avoid:
- “MAC randomization always creates a new IP”
- “DHCP identifies devices only by MAC”
- “MAC randomization causes DHCP exhaustion”
- “modern phones change MAC every reconnect”
- “first RCM-aware DHCP study”
- “new lease amplification metric”

Use:
> **For analytical purposes, lease amplification is operationalized as the ratio of valid DHCP leases to active physical clients.**
...
# Adviser-Ready Status and Remaining Decisions

This is **Proposal Draft V2 — Adviser Ready**.

Items intentionally left provisional until adviser feedback or a focused methodology pilot:
- exact mathematical definition of 50%, 75%, and 90% pool pressure;
- final literature-backed guest-network arrival/session distribution;
- final warm-up duration and measurement horizon;
- exact confidence-interval precision stopping rule;
- department-required reference style;
- whether the adviser prefers the full title or a shorter alternative.

### Recommended shorter fallback title

**Computational Modeling of DHCPv4 Lease Utilization under DHCP-Visible Identity Turnover in Constrained Guest Networks**

Use the full title by default because it exposes both MAC randomization and Client-Identifier continuity. Use the shorter version only if the adviser prioritizes title brevity.

### Immediate next technical-writing task

After adviser approval of the topic proposal, convert the approved wording into Chapter 1 sections without changing the research scope:
...
The simulator implementation should begin only after the main state machine and experiment-parameter specification are frozen.
