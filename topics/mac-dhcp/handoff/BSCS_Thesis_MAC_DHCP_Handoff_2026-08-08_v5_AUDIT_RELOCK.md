...
# 85. HANDOFF UPDATE — AFTER METHODOLOGY DRY-RUN v0 AND KEA VALIDATION DESIGN

This section supersedes any older “current status,” “current design,” “next step,” or “lock condition” wording above wherever there is a conflict. Earlier sections remain useful as the historical research trail.

## 85.1 CURRENT STATUS

The topic remains the strongest surviving candidate and has now passed an initial **simulation dry-run / mechanism check**.

Current status:

> **PROPOSAL-CAPABLE, WITH SIMULATION DRY-RUN PASSED; ACTUAL KEA PACKET-LEVEL VALIDATION STILL PENDING.**

What has now been completed conceptually/experimentally:
- protocol semantics re-checked against RFC 2131;
- Client-ID/privacy logic re-checked against RFC 7844;
- standards-informed identity behavior retained from RFC 9724;
- small discrete-event DHCP model dry-run performed;
- persistent-vs-rotating Client-ID behavior tested in the simulator;
- release-vs-silent-departure mechanism tested;
- lease-duration and pool-pressure interactions observed in synthetic stress tests;
- Kea lease-matching behavior verified from current documentation;
- reproducible Kea validation topology designed;
- Scapy identified as the preferred exact identity-scenario generator;
- perfdhcp retained as a secondary load/overhead validation tool.

Actual Kea execution was **not** completed inside the prior AI runtime because that environment did not permit Linux network namespace creation and package setup did not complete. This is an environment limitation, not evidence that the design is infeasible.

Do NOT state that Kea validation has already been executed.

---

# 86. IMPORTANT DRY-RUN FINDING: THE OLD 3 × 2 IDENTITY FACTORIAL WAS REDUNDANT

Older plan:
- MAC behavior: Stable / Periodic / Session
- Client-ID: Persistent / Rotating

This created six combinations.

The dry-run exposed a conceptual redundancy:

If “rotating Client-ID” means the Client-ID changes **together with a MAC identity-change event**, then:

- Stable MAC + Persistent CID
- Stable MAC + Synchronized Rotating CID

are identical because there is no identity-change event to trigger the Client-ID rotation.

Therefore, the main experiment should no longer be a full 3 × 2 crossed design.

The cleaner structure is five **identity profiles**:

## P0 — Baseline
Stable/per-network MAC + persistent Client-ID
...
This is currently the preferred main identity design.

The effect of Client-ID continuity can still be analyzed through planned contrasts:
- **P1 vs P2**
- **P3 vs P4**

P0 remains the stable control.

---

# 87. REVISED MAIN EXPERIMENT SIZE
...
= **60 main configurations**

Lease durations remain provisionally:
- 1 hour
- 2 hours
- 8 hours
- 24 hours

Pool-pressure levels remain provisionally:
- 50%
- 75%
- 90%

Replication plan remains:
- minimum 30 seeds;
- continue if required for confidence-interval precision.

The previous 72-configuration plan should be considered superseded.

---

# 88. IMPORTANT DRY-RUN FINDING: “POOL EXHAUSTION” WAS TOO AMBIGUOUS

The dry-run showed that reaching 100% allocated addresses is not the same as actual service failure.

A DHCP pool may briefly become fully allocated without denying a client if no new allocation request arrives during that interval.

Therefore, the old broad metric “pool exhaustion probability” should be split.

## Revised metrics

### Pool saturation probability
Did valid allocated leases ever reach the address-pool capacity during the observation period?

### Pool saturation time fraction
What fraction of measured time had zero free addresses?

### Allocation failure rate
What fraction of actual address-allocation attempts failed because no address was available?

### Time to first saturation

### Time to first allocation failure

This distinction is important for technical correctness and panel defense.

Do not equate:
> “pool reached 100%”

with:
> “clients were denied service.”

---

# 89. CORE PHENOMENON REFINED

The dry-run clarified that:

> **MAC turnover alone does not necessarily create DHCP lease amplification.**

If a persistent DHCP Client Identifier allows the server to recognize the same logical client after a MAC change, lease continuity can remain intact.
...
RCM / MAC turnover
        ↓
DHCP-visible logical identity turnover
        ↓
loss of lease continuity
        ↓
valid-lease amplification
        ↓
pool saturation risk
        ↓
allocation failure (only if a request arrives with no free address)
```

This is more accurate than:
> “Changing MAC consumes IPs.”

The thesis should increasingly be framed around:
> **DHCP-visible identity continuity / turnover**

rather than MAC churn alone.

---

# 90. WORKING DEFINITION: LEASE AMPLIFICATION

For analytical purposes, the phenomenon can be operationalized through:

> **valid-lease-to-active-client ratio**

Conceptually:

`Lease Amplification = valid DHCP leases / active physical clients`

Interpretation:
- 1.0 = roughly one valid lease per active physical client
- >1.0 = server retains more valid leases than active physical clients

Do not present this as a standardized or revolutionary new networking metric.

Preferred wording:
> “For analytical purposes, lease amplification is operationalized using the valid-lease-to-active-client ratio.”

---

# 91. SYNTHETIC DRY-RUN RESULTS — FOR INTERNAL VALIDATION ONLY

A small synthetic stress test was performed in the prior conversation.

Important:
> **These numbers are NOT thesis findings and must never be copied into Chapter 4 or a formal proposal as empirical results.**

They were used only to check whether the proposed mechanism behaves coherently.

Representative synthetic observations included:
- persistent Client-ID greatly reduced lease accumulation under changing MAC identities;
- rotating Client-ID produced much greater lease retention under silent departures;
- longer leases produced more retained valid lease state than shorter leases;
- higher physical demand made the consequences more severe;
- 100% proper DHCPRELEASE eliminated the artificial lease accumulation in the tested control condition.

The dry-run therefore supported the expected mechanism and, more importantly, exposed the design corrections in Sections 86–89.

Do not preserve or cite the synthetic percentages as research evidence.

---

# 92. RELEASE CONTROL — CURRENT INTERPRETATION

A strong falsification/control test emerged from the dry-run.
...
old leases can remain valid until expiry.

Under:
- changing MAC;
- changing Client-ID;
- proper DHCPRELEASE before identity change/session departure;

lease accumulation should largely disappear.

This control is valuable because it demonstrates that the study is not assuming:
> “MAC change automatically consumes another IP forever.”

Instead, the phenomenon depends on:
> **identity turnover + retained valid lease state**

This is consistent with the RFC 2131 normative release model.

---

# 93. SECONDARY IDENTITY CONDITION: NO CLIENT-ID / CHADDR-ONLY

RFC 2131 permits DHCP identity matching through `chaddr` when Client Identifier is absent.

A possible secondary/sensitivity condition is therefore:

> **No Client-ID / chaddr-only identification**

This should NOT be added to the 60 main configurations unless needed.

Use it as a secondary validation/sensitivity test to show that the resource effect is not tied only to one encoding of Option 61.

When `chaddr` alone identifies the client, changing MAC naturally changes DHCP-visible logical identity.

---

# 94. KEA VALIDATION — CURRENT TECHNICAL DESIGN

The validation lab can be built inside a single ordinary Linux VM.
...
No real Wi-Fi network is needed.

No special NIC is required.

No production LAN should be used for validation traffic.

---

# 95. WHY SCAPY IS NOW PREFERRED FOR IDENTITY-SEMANTICS VALIDATION

`perfdhcp` is useful for:
- multiple simulated clients;
- controlled request rate;
- renewals;
- releases;
- load testing;
- repeatable large-client tests.

However, the exact thesis mechanism requires precise control over:
...
- server identifier;
- RELEASE behavior;
- deliberate sequence representing “same physical client, different network identity.”
...
Purpose:
Verify exact MAC/CID identity semantics.

## Level 2 — Quantitative validation
Use **Simulator ↔ Kea/perfdhcp**

Purpose:
Compare selected time-series/aggregate DHCP resource metrics.

This two-level validation strategy supersedes the earlier vague statement that Kea would simply “validate selected scenarios.”

---

# 96. DETERMINISTIC KEA VALIDATION TEST SET

Current preferred micro-tests:

## V0 — Baseline
Same MAC + same Client-ID on repeated connection.

Expected:
same logical client / same lease continuity.

## V1 — MAC rotation, stable Client-ID
MAC changes, CID stays the same.
...
Expected:
Kea should recognize the same logical client through Client-ID when `match-client-id=true`.

## V2 — MAC + Client-ID rotation
Both MAC and CID change.
...
Expected:
separate logical identities; old valid leases may remain without RELEASE.

## V3 — Tiny-pool saturation/failure
Use a 3-IP pool and issue four distinct rotating MAC/CID identities before old leases expire.

Purpose:
Observe actual Kea behavior when logical identity turnover exceeds available addresses.

Do not assume failure beforehand; record what Kea actually does.

## V4 — DHCPRELEASE control
Same turnover as V2, but release the prior lease before moving to the next identity.

Expected:
lease accumulation should largely disappear.

## V5 — chaddr-only / MAC identity
Use no Client-ID or set `match-client-id=false`.

Purpose:
Validate behavior when MAC/chaddr is the primary logical identity.

---

# 97. STRONGEST FIRST REAL KEA TEST

The recommended first real lab check is deliberately tiny.

Use:
- 3 available IP addresses;
- 1-hour lease;
- no release initially.

### Continuity case
One conceptual physical client:
- MAC A / CID X
- MAC B / CID X
- MAC C / CID X
- MAC D / CID X

Observe whether Kea keeps continuity through CID.

### Identity-turnover case
Same conceptual physical client:
- MAC A / CID A
- MAC B / CID B
- MAC C / CID C
- MAC D / CID D
...
Then repeat with proper DHCPRELEASE.

This provides a very compact validation of the entire thesis mechanism.

---

# 98. KEA CONFIGURATION ELEMENTS TO USE

The validation design should include approximately:

- `interfaces-config`
- Memfile lease backend
- `match-client-id: true` for Client-ID tests
- `match-client-id: false` for chaddr-only sensitivity test
- controlled `valid-lifetime`
- explicit `renew-timer`
- explicit `rebind-timer`
- small fixed subnet/pool
- isolated temporary lease-data directory

Use current Kea documentation before coding the final file.

Do not rely on obsolete configuration semantics from old versions.

---

# 99. KEA STATISTICS RELEVANT TO VALIDATION

The Kea statistics subsystem can provide metrics such as:
- assigned-addresses
- cumulative-assigned-addresses
- reclaimed-leases
- per-subnet/per-pool assignments
- allocation-failure counters
- packet counters

Where possible, validation should compare the simulator to Kea’s own server state/statistics rather than reconstructing everything externally.
...
This makes the validation more defensible.

---

# 100. UPDATED SPECIFIC OBJECTIVE 3

Preferred revised wording:

> **To develop an isolated DHCPv4 validation environment that uses programmable client identity generation and an open-source DHCP server to verify the modeled effects of MAC identity turnover, Client-Identifier continuity, lease retention, renewal, release, and address-pool saturation.**
...
# 101. TECHNICAL FEASIBILITY STATUS

Current technical feasibility assessment:

- Python discrete-event simulation: **feasible**
- identity-profile logic: **feasible**
- RFC-consistent DHCP state model: **feasible**
- Scapy exact packet generation: **feasible in a normal privileged Linux VM**
- Kea DHCPv4 validation server: **feasible**
- perfdhcp secondary load testing: **feasible**
- Linux namespace topology: **feasible on a normal Linux environment with appropriate privileges**
- special Wi-Fi hardware: **not required**
- production university network: **not required**
- real student/user data: **not required**
- paid services: **not required**

Estimated additional cost remains near zero if an ordinary laptop/PC is already available.

---

# 102. TOPIC LOCK CONDITION — REVISED

The prior lock condition required actual Kea execution.

Current strongest conservative lock rule:

Topic can be marked **LOCKED** when:
1. no fatal directly overlapping peer-reviewed paper is found;
2. simulation dry-run remains coherent;
3. five-profile identity design is accepted;
4. Scapy/Kea validation pack is complete and syntax-reviewed;
5. at least one actual Kea micro-test is run successfully on a normal Linux environment;
6. V1 and V2 show interpretable identity-continuity differences or otherwise reveal a correctable model discrepancy;
7. cost remains near zero;
8. user/group understands the core concept well enough to defend it.

Until actual packet-level Kea testing occurs, use:
> **proposal-capable, pending final practical validation**
...
# 103. CURRENT NEXT STEP — SUPERSEDES OLDER NEXT STEPS

Before polished Chapter 1 / Topic Proposal writing:

> **Build the reproducible Kea validation pack.**

The pack should contain:
- namespace setup script;
- namespace cleanup script;
- Kea DHCPv4 configuration;
- temporary lease-data directory setup;
- Scapy DHCP client/identity generator;
- V0–V5 deterministic test cases;
- result logger;
- Kea statistics query/check procedure;
- optional tcpdump/Wireshark capture command;
- execution instructions;
- expected qualitative behavior;
- troubleshooting notes.

The pack can be syntax-checked in the AI environment even if the environment itself cannot create namespaces.

After the pack is ready, run it on a normal Linux VM.

If the real Kea tests support or meaningfully refine the model, then:
> **LOCK THE TOPIC and proceed directly to the official Topic Proposal.**

---

# 104. GROUP COMMUNICATION PREFERENCE

The user requested that explanations for groupmates be:
- **Taglish**
- simple and non-condescending;
- clear even when introducing networking concepts;
- free of wording that implies the group “does not know” networking/cybersecurity.

Avoid statements like:
> “for people who do not understand networking”

Prefer neutral phrasing like:
> “quick guide para pare-pareho tayong malinaw sa main idea.”

A Taglish group-friendly PDF was created in the prior conversation:
> **Simple_Thesis_Explanation_Taglish_v2.pdf**

The explanation centers these analogies:
- MAC = name tag
- Client-ID = ID card
- IP address = room number
- DHCP = receptionist
- lease = reservation time

The user may reuse this explanation style for group discussions and presentation prep.

---

# 105. SIMPLE TOPIC EXPLANATION TO PRESERVE FOR FUTURE CONTINUATION

Very simple explanation:

> **Pag-aaralan natin kung kailan nakakaapekto ang changing or randomized network identity ng devices sa DHCP IP leases. Pwedeng isang actual device lang ang meron, pero kung nagbabago ang identity na nakikita ng DHCP at valid pa ang old leases, pwedeng mas maraming IP addresses ang naka-reserve kaysa sa actual active devices. Susukatin natin kung kailan maliit lang ang effect at kung kailan ito nagiging problem, depende sa identity continuity, lease duration, at kung gaano kapuno ang IP pool.**

Very short answer to:
> “Ano thesis natin?”

> **“Sinusukat natin kung paano nakakaapekto ang changing or randomized network identities sa DHCP IP leases. Tinitingnan natin kung kailan pwedeng dumami ang valid leases kumpara sa actual active devices, at iva-validate natin ang simulation gamit ang real DHCP software.”**

---

# 106. CURRENT ONE-SENTENCE STATUS — LATEST

> **The thesis is now a proposal-capable, near-zero-cost computational modeling study centered on DHCP-visible identity continuity: the simulator dry-run passed and exposed useful design corrections, the main experiment has been simplified to five identity profiles × four lease durations × three pool-pressure levels, and the final remaining practical gate before topic lock is an actual Scapy-to-Kea packet-level validation in an isolated Linux environment.**

---

# 107. UPDATED CONTINUATION PROMPT FOR A NEW AI — LATEST

If the conversation is lost, upload this handoff and say:

> **“Continue from this handoff. Use the latest sections 85–107 as the source of truth where they conflict with older sections. Do not write the polished proposal yet. Build the reproducible Kea validation pack: namespace setup/cleanup scripts, Kea config, Scapy DHCP identity generator, V0–V5 deterministic tests, result logging, and validation instructions. Keep explanations Taglish and non-condescending when discussing the topic with the group. Continue novelty surveillance, but do not reopen rejected broad topics unless fatal overlap appears. After at least one real Kea micro-test succeeds and no fatal academic overlap is found, lock the topic and draft the official CS 124 Topic Proposal.”**

---

# 108. FINAL HANDOFF UPDATE — TOPIC LOCKED AFTER REAL KEA VALIDATION

This section supersedes older status/lock wording wherever there is a conflict.

## CURRENT STATUS

> **TOPIC LOCKED**

The final practical gate was completed successfully using a real Kea DHCPv4 3.2.0 server inside WSL2 Ubuntu, an isolated namespace/veth lab, and a Scapy 2.7.0 programmable DHCP client.

These are **mechanism-validation results**, not Chapter 4 main thesis findings. They validate that the computational model's identity/lease semantics are technically sound, but they do not prove that RCM commonly causes DHCP exhaustion in real guest networks.

# 109. REAL KEA VALIDATION — FINAL RESULTS

Environment:
- WSL2 Ubuntu
- Kea DHCPv4 3.2.0
- Scapy 2.7.0
- pool: 10.77.0.100–10.77.0.102
- valid lifetime: 3600 s
- T1: 1800 s
- T2: 3150 s

## V0 — Same MAC + Same Client-ID
Same identity reused 10.77.0.100; final assigned-addresses = 1; allocation failures = 0.

> **SUPPORTED — normal lease continuity works.**

## V1 — Changing MAC + Stable Client-ID
MAC 01 / CID X, MAC 02 / CID X, and MAC 03 / CID X all reused 10.77.0.100; final assigned-addresses = 1.

> **SUPPORTED — stable Client-ID can preserve DHCP logical identity despite MAC turnover.**

## V2 — Changing MAC + Changing Client-ID
MAC 01 / CID A → .100; MAC 02 / CID B → .101; MAC 03 / CID C → .102; final assigned-addresses = 3.

> **SUPPORTED — DHCP-visible identity turnover can create additional simultaneously valid lease state.**

## V3 — Four Distinct Identities / Three-IP Pool
First three identities obtained leases; fourth identity received no offer; v4-allocation-fail = 1.

> **SUPPORTED — retained lease state can create pool pressure and actual allocation failure.**

Critical distinction:
> Pool saturation is not itself service failure; failure occurs when a new allocation request arrives while no address is free.

## V4 — DHCPRELEASE Control
Three changing identities acquired and properly released their leases; final assigned-addresses = 0.

> **SUPPORTED — proper RELEASE removes/reduces retained lease pressure.**

## V5 — chaddr-Only Matching
With match-client-id=false, same CID X plus three different MACs produced three different IP leases.

> **SUPPORTED — when chaddr is used for matching, MAC turnover changes DHCP-visible identity.**

# 110. MODEL VALIDATION VERDICT

All six core assumptions are supported:
1. Same DHCP logical identity preserves lease continuity.
2. Stable Client-ID can preserve continuity despite MAC change.
3. Rotating MAC + rotating Client-ID can create additional valid lease state.
4. Retained leases can create address-pool pressure.
5. Proper RELEASE removes/reduces retained lease pressure.
6. chaddr-only matching makes MAC turnover relevant to DHCP identity.

Overall:
> **MODEL MECHANISM VALIDATED FOR TOPIC LOCK**

# 111. TERMINOLOGY CORRECTION

Avoid:
> “Logical identity = (Client-ID, MAC) pair.”

Preferred:
> **DHCP logical identity is determined by the identifier used by the server's lease-matching behavior. When Client-ID matching is enabled, a persistent Client Identifier can preserve lease continuity despite MAC changes; when Client-ID is unavailable, rotated, or ignored, chaddr/MAC changes can produce new DHCP-visible identities.**

# 112. FINAL LOCKED THESIS TITLE — PROPOSAL DEFAULT
...
Total:
> **5 × 4 × 3 = 60 main configurations**

Replication:
- minimum 30 independent seeds
- additional replications based on 95% CI precision if needed
...
# 115. LOCKED PRIMARY METRICS

- peak/mean lease utilization
- pool saturation probability
- pool saturation time fraction
- allocation failure rate
- time to first saturation
...
Use as an analytical operationalization, not as a claimed standardized metric.

# 116. FINAL CS POSITIONING
...
- **Computing Contribution:** Standards-informed, implementation-validated discrete-event DHCPv4 lease-utilization model with interaction/threshold characterization
- **BU Agenda:** Institutional Development and Policy Innovation
- **SDG:** SDG 9 — Industry, Innovation and Infrastructure
- **Beneficiary:** Network administrators managing constrained guest/public networks
- **Expected Innovation:** Reproducible characterization of DHCP-visible identity turnover × Client-ID continuity × lease duration × pool pressure

# 117. NEXT PHASE — PROPOSAL WRITING

Topic hunting is finished unless a directly overlapping paper is later discovered.

Next:
1. Draft the official CS 124 Topic Proposal.
2. Stabilize title, gap, Background, Proposed Model, General Objective, Specific Objectives, and Scope/Limitations.
3. Build RRL source map.
4. After proposal wording stabilizes, implement the full simulator.

# 118. REAL VALIDATION ARTIFACTS

Authoritative implementation-level validation record:
- `VALIDATION_RESULTS.md`
- `validation_summary.json`
- local V0–V5 PCAPs
- client JSON outputs
- Kea statistics snapshots
- Kea logs
- lease CSV

# 119. CURRENT ONE-SENTENCE STATUS

> **The thesis topic is LOCKED: real Kea 3.2 validation in WSL2 supported all six core DHCP identity/lease assumptions, so the project now moves from topic validation into formal CS 124 proposal writing and full simulator design.**

# 120. LATEST CONTINUATION PROMPT

> **Continue from this handoff using Sections 108–120 as the latest source of truth. The thesis topic is LOCKED after successful real Kea 3.2 V0–V5 validation. Do not reopen broad topic hunting. Continue with the official CS 124 Topic Proposal using the uploaded Topic Proposal Template and Thesis Domains guide. Keep novelty claims cautious: characterize the contribution as a controlled quantitative characterization of DHCP-visible identity turnover × Client-ID continuity × lease duration × pool pressure, not as the first RCM-aware DHCP work. Treat V0–V5 as mechanism validation, not final Chapter 4 findings. After proposal wording is stable, proceed to the full discrete-event simulator.**

---

# 121. FINAL DEEP NOVELTY AUDIT — SUPERSEDES THE PREVIOUS BROAD LOCK

**Date:** 2026-08-08

The user requested one final adversarial deep-research audit specifically to determine whether the thesis repeats prior work, whether the novelty is real-world acceptable, and whether the topic should survive.

## FINAL AUDIT DECISION

> **REVISE / RE-LOCK — DO NOT KILL**

The topic family survives, but the previous broad “TOPIC LOCKED” novelty framing is superseded.

The earlier framing was too optimistic because close prior art exists in standards, a 2015 IEEE experiment, contemporary RCM literature, vendor guidance, and Cisco/ARRIS patents.

---

# 122. WHAT IS DEFINITELY NOT NOVEL

Do not claim novelty for any of these:

1. RCM / randomized MAC behavior affects DHCP.
2. RCM can contribute to DHCP pool/scope exhaustion.
3. Stable DHCP Client Identifier can preserve IP/lease continuity across MAC rotation.
4. Changing DHCP-visible identity can create additional leases.
5. Shorter DHCP leases can reduce retained-address pressure.
6. Lease duration affects address utilization and DHCP overhead.
7. Active-leases / active-clients ratio as a new metric.
8. Thresholding active-leases-vs-active-clients divergence.
9. General relation among pool size, lease time, client count, and MAC-rotation frequency.
10. Privacy-versus-stable-identifier continuity as a newly discovered trade-off.
11. Generic DHCP modeling/simulation.
12. Generic RCM-aware DHCP optimization.

These are all established somewhere in standards, peer-reviewed work, operational guidance, or patent prior art.

---

# 123. CLOSEST PRIOR ART FOUND

## Cisco US11483283B1
“DHCP resource optimization for randomized and changing MAC address”

Explicitly uses stable identifiers / DHCP Option 61 to preserve an IP binding across MAC rotations.

Implication:
> V1 proves feasibility/implementation validity, not novelty.

## Cisco US11962567B2
“Address rotation aware dynamic host control protocol”

Explicitly describes DHCP exhaustion as a function of:
- address-pool size;
- lease time;
- number of clients;
- MAC rotation frequency.

Also defines divergence/ratio between active IP addresses/leases and active clients and uses thresholds/control loops.

Implication:
> valid-leases/active-clients ratio and threshold ideas are not novel.

## ARRIS US11765128B2
“DHCP server IP address allocation improvement to nullify the impact of mac randomization”

Explicitly proposes shorter lease duration for randomized-MAC clients.

Implication:
> never propose “shorten randomized client lease” as the innovation.

## Bernardos, Zúñiga, O’Hanlon — IEEE CSCN 2015
Historical IETF/IEEE RCM experiments already used DHCP Client Identifier, observed DHCP/IP allocation effects, and introduced shorter DHCP leases in later trials.

Implication:
> simply testing RCM × CID × lease duration is insufficiently novel.

---

# 124. WHAT SURVIVED THE AUDIT

The following narrower contribution survived targeted searches:

> **A standards-informed, implementation-validated computational characterization of how persistent versus privacy-synchronized DHCP Client-Identifier behavior changes lease-state overhead, address-pool saturation, and allocation-failure risk under controlled randomized-MAC turnover, lease-duration, and exogenous offered-load conditions.**

No exact peer-reviewed duplicate was located whose central controlled design jointly manipulates:
1. standards-informed RCM identity behavior;
2. persistent vs privacy-synchronized rotating DHCP CID under matched MAC behavior;
3. multiple lease durations;
4. exogenous offered physical-client load / constrained pool capacity;
and then estimates interaction/operating regions using replicated computational simulation with implementation-level DHCP validation.

This is **absence from the searches performed, not proof of universal firstness**.

---

# 125. FINAL NOVELTY POSITION AFTER AUDIT

Primary scientific question:

> **What is the quantitative DHCPv4 resource cost of privacy-synchronized DHCP identity rotation compared with persistent Client-Identifier continuity under the same randomized-MAC behavior, and under what lease-duration and offered-load conditions does that cost transition from negligible to service-impacting?**

Core novelty:
> **interaction / response-surface / operating-region characterization**

Not novelty:
> mechanism discovery, metric invention, DHCP optimization, or RCM mitigation invention.

---

# 126. REVISED RESEARCH GAP — AUDIT-SAFE
...
# 127. REVISED TITLE — RE-LOCKED RECOMMENDATION
...
The preferred title is narrower and maps directly to the contribution that survived the final novelty audit.

---

# 128. REQUIRED METHODOLOGY CHANGES AFTER AUDIT

## 128.1 Client-ID continuity becomes the scientific center

Primary paired contrasts:
- same MAC-turnover process + persistent Client-ID
- same MAC-turnover process + privacy-synchronized rotating Client-ID

The difference isolates DHCP logical-identity continuity under matched RCM behavior.

## 128.2 Replace “pool pressure” as an experimental-factor name

Current lease utilization is an outcome and should not define an independent variable.

Use:

> **Offered physical-client load = offered active physical-client demand / DHCP pool capacity**

Provisional levels may remain 50%, 75%, 90% after precise workload parameterization.

## 128.3 Metric terminology

Use:
> **valid-lease-to-active-client ratio**

It is a descriptive response metric, not a new metric or invention.

## 128.4 Reclassify per-session P3/P4

RFC taxonomy permits per-session generated MAC behavior, but current mainstream Android/iOS defaults do not make “new MAC every reconnect” a universal assumption.

Treat P3/P4 as:
> **stress/sensitivity / upper-bound RCM profiles**

not equal-status default real-world profiles.

## 128.5 RELEASE limitation

Kea V4 showed immediate/near-immediate pressure removal, but RFC 9797 says DHCP servers may retain a released IP for a period.

Do not generalize Kea release behavior to all implementations.

At minimum document this as a limitation; preferably add release-retention sensitivity if feasible.

## 128.6 Optional second DHCP implementation

A small subset of V0–V5 on a second DHCP implementation would strengthen external validity and reduce “Kea-specific” criticism.

Optional, not required to retain feasibility.

---

# 129. RECOMMENDED PRIMARY EXPERIMENT AFTER AUDIT

Cleaner primary inferential experiment:

- P0 — stable/per-network MAC + persistent CID
- P1 — periodic MAC turnover + persistent CID
- P2 — matched periodic MAC turnover + privacy-synchronized rotating CID
- 4 lease durations
...
Then use P3/P4 per-session turnover at selected operating points as sensitivity/stress tests.

Reason:
- cleaner causal/scientific contrast;
- stronger novelty story;
- less risk of implying per-session MAC rotation is the normal default on phones;
- still computationally substantial and manageable.

The old 60-config design remains feasible but is no longer the preferred scientific framing.

---

# 130. REAL-WORLD RELEVANCE AFTER AUDIT

The application domain remains defensible:

- RFC 9797 explicitly treats public guest networks as a low/zero-trust environment where long-lived MAC identity should be avoided.
- Current Apple behavior uses Rotating private Wi-Fi addresses by default on weaker/no-security/open/captive environments.
- Android uses persistent randomization by default and conditional non-persistent behavior in some cases; one condition is explicitly coupled to DHCP lease expiration.
- Cisco operational guidance documents temporary DHCP-pool impact from changing/randomized MAC identities.

Therefore:
> guest/public networks are a credible application domain, but the simulator must describe its identity profiles as controlled standards-informed abstractions rather than exact emulation of every mobile OS.

---

# 131. BSCS NOVELTY / ACCEPTABILITY VERDICT

Qualitative assessment after adversarial audit:

- Basic mechanism novelty: **weak / none**
- Exact duplicate risk after reframe: **low-to-moderate**
- BSCS originality if rigorously executed: **moderate-to-strong**
- Real-world relevance: **strong**
- Technical feasibility: **very strong**
- Cost: **low**
- Panel risk with old broad framing: **high**
- Panel risk with revised framing: **manageable**
- Publication-level novelty: **moderate, not breakthrough**
- “first ever” claim support: **none**

The official course rules do not require a world-first mechanism; they require original contribution, rigorous research, and allow confirmation/rejection of prior findings as meaningful when properly designed. Computational Modeling and Simulation is explicitly an acceptable substantial CS component.

---

# 132. FINAL STATUS SENTENCE — USE THIS GOING FORWARD

> **The thesis is RE-LOCKED after the final deep novelty audit: the known RCM/DHCP mechanisms are treated as prior knowledge, while the research contribution is narrowed to a replicated, implementation-validated computational characterization of the DHCPv4 lease-state operating regions created by persistent versus privacy-synchronized Client-Identifier behavior across lease-duration and offered-load conditions in constrained guest networks.**

---

# 133. FINAL AUDIT ARTIFACT

Detailed report:
`FINAL_DEEP_NOVELTY_AUDIT_DHCP_RCM_2026-08-08.md`

This report supersedes earlier statements that the broad RCM × DHCP × Client-ID mechanism itself was novel.

---

# 134. NEXT ACTION AFTER USER ACCEPTS AUDIT

Do not continue Chapter 1 from Proposal Draft V2 unchanged.

Create **Proposal Draft V3 — Audit Reframed** using:
- revised title;
- revised gap;
- Client-ID privacy/continuity contrast as central research question;
- “offered load” instead of “pool pressure” as independent factor;
- 36-config primary experiment + P3/P4 sensitivity;
- Kea V0–V5 as mechanism validation only;
- patent prior art explicitly acknowledged in RRL/novelty boundaries.
